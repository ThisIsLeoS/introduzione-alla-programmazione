			...
			while (true)
			{
				...
				b=trim(s); // viene chiamata prima "trim" di "lower", così potenzialmente verrà passata una stringa più corta a quest'ultima funzione e il calcolatore farà meno lavoro
				...
				else 
			  	{
					lower(s);
			  		add_word(az, s);
			  		change_next_couple(az, s);
				  	cout << "La parola è stata aggiunta\n";
					break; // per uscire dallo "while (true)" più annidato
				}
			}
		}
		else if (c=='l') 
		...
		{		
			while (true)
			{
				...
				if (c=='u') 
				{
					...
					break; // per uscire dallo "while (true)" meno annidato
				}
				else if (c=='d')
				{
					while (true)	
					{	  		  			
						cout << "Inserisci le due lettere: ";
						cin >> s;
						cout << endl;
						if (s.size()<2 || s.size()>2) cout << "Devi inserire due caratteri\n\n"; // se sono stati inseriti meno o più di due caratteri
			  			else
						{
							print_word_by_couple(az,s);
							break; // per uscire dallo "while (true)" più annidato
						}
					}
					break; // per uscire dallo "while (true)" meno annidato
				}
				else cout << "Devi inserire o 'u' o 'd'\n\n";
			}
		}
